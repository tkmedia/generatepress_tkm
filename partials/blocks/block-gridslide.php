<?php
$uid = $block['id'];
$className = 'acftkm_gridslide_block';
if( !empty($block['className']) ) {
   $className .= ' ' . $block['className'];
}

$grid_source = get_field('fg_src');
$grid_type = get_field( 'fg_tp' );
$grid_count = get_field('fg_cu');
$grid_count_mobile = get_field('fg_cum');
$grid_slider = get_field('fg_sl');
$grid_slidermo = get_field('fg_slm');

$grid_ststyle = get_field('fg_ststy');
$grid_style = get_field('fg_cgs');
$grid_imageposition = get_field('fg_cip');
$grid_custom_size = get_field('fg_cis');
$grid_imageheight = get_field('fg_cih');
$grid_item_titlesize = get_field( 'fg_its' );
$grid_item_subtitlesize = get_field( 'fg_ists' );
$grid_item_titlecolor = get_field( 'fg_itc' );
$grid_item_subtitlecolor = get_field( 'fg_istc' );
$grid_pad = get_field( 'fg_ipa' );
$grid_grid = get_field( 'fg_gd' );
...
<div id="<?php echo $uid; ?>" class="flex_content_cols <?php echo esc_attr($className); ?>">
	...
	#<?php echo $uid; ?> .masonary_grid_link.grid_features_icon .layout .grid-item .grid-item-inner-img.contain-object-fit {height:<?php echo $grid_imageheight; ?>px;}
	#<?php echo $uid; ?> .masonary_grid_link .grid-item {padding:<?php echo $grid_pad; ?>px;}	
	<?php } ?>
	<?php if( $grid_style == 'stories' ) { ?>
	@media (min-width: 768px) {
		#<?php echo $uid; ?> .masonary_grid_link.box-layout.grid_stories .flex_masonary_content_wrap {min-height:<?php echo $grid_imageheight ?>px;}
	}
	<?php } ?>
	</style>
	...
	<section id="section-<?php echo $uid; ?>" class="page_flexible section_flex_feature page_flexible_content section-<?php echo $uid; ?>">
		...
		<div class="masonary_grid_link row_items_<?php echo $grid_count; ?><?php if( $grid_source == 'manual' ) { ?> <?php echo $grid_type; ?> grid_<?php echo $grid_style; ?><?php } elseif( $grid_source == 'gallery' ) { echo $grid_type; } ?> flexible_page_element source_<?php echo $grid_source; ?>" itemprop="text">

			<?php 
			if( $grid_source == 'manual' ) { 
			$page_grid_overlap = get_field( 'fg_oltm' );
			?>

			<div class="masonary_grid_link_wrap masonary_grid-<?php echo $uid;?><?php if( $grid_type == 'box-layout' ) { ?> <?php echo $grid_imageposition; }?>"<?php if( $page_grid_overlap ) { ?> style="margin-top:-<?php echo $page_grid_overlap;?>px;"<?php } ?>>
				...
				<div class="masonary_grid<?php if( $grid_slider ) { ?> slider-container<?php } ?><?php if( $grid_slidermo ) { ?> slider-container-mo<?php } ?>">
					...
					foreach($grid_grid as $gridrow) {
						$masonary_img = $gridrow['m_im'];$masonary_ico = $gridrow['m_icon'];$masonary_title = $gridrow['m_t'];$masonary_subtitle = $gridrow['m_st'];$masonary_text = $gridrow['m_stxt'];$masonary_linktype = $gridrow['m_lkt'];$masonary_link = $gridrow['m_lk'];$masonary_linkpage = $gridrow['m_plk'];$masonary_linkform = $gridrow['m_flk'];$masonary_bottomtext = $gridrow['m_bot'];$masonary_newtab = $gridrow['m_bot'];$masonary_button_text = $gridrow['m_btnt'];$masonary_con_position = $gridrow['m_cop'];$masonary_over = $gridrow['m_over'];$masonary_tic = $gridrow['m_tic'];$masonary_stc = $gridrow['m_stc'];$masonary_bgc = $gridrow['m_ibg'];//$masonary_title_color = $gridrow['m_tcl'];
					?>
					    <div class="grid-item <?php if( $grid_slider ) { ?>slider-slide<?php } else { ?>col-xs-<?php echo $m_xs_cols; ?> col-sm-<?php echo $m_sm_cols; ?> col-md-<?php echo $m_md_cols; ?> col-lg-<?php echo $m_lg_cols; ?><?php } ?>">
							...
							<a class="img_info_link" data-fancybox data-src="#popop-form-<?php echo $uid;?>-<?php echo $link_item;?>" href="javascript:;">		
							<?php endif; ?>
							
							
							<div class="grid-item-inner<?php if( $masonary_over ) { ?> item-overlay<?php }?>"<?php if( $masonary_bgc ) { ?> style="background-color:<?php echo $masonary_bgc;?>;padding: 15px;"<?php }?>>
								<?php if( $grid_style == 'testimonials' ) { ?>
									...
									<div class="flex_masonary_text grid-item-<?php echo $uid;?>" style="color:<?php if($masonary_tic) { echo $masonary_tic; } else { echo $grid_item_titlecolor; } ?>;"><?php echo $masonary_text; ?></div>
									<?php } ?>
								...
								<div class="grid-item-inner-img-bg" style="<?php if( $masonary_img && $grid_imageheight ) { ?>height:<?php echo $grid_imageheight; ?>px;<?php } else { ?>height:auto;<?php } ?>">
									
									<?php if( $grid_style == 'features_icon' && $masonary_ico ) { 
										$masonary_icoc = $gridrow['m_icoc']; ?>
									<div class="grid-item-inner-ico" style="font-size:<?php echo $grid_imageheight; ?>px;color:<?php echo $masonary_icoc; ?>;">
									...
									<div class="flex_masonary_content<?php if( $grid_style == 'simple' && $masonary_con_position ) { ?> con_<?php echo $masonary_con_position; } ?>" style="color:<?php //echo $masonary_title_color; ?>;">
										<div class="flex_masonary_content_container">
											<div class="flex_masonary_content_wrap">
												...
												<div class="flex_masonary_text grid-item-<?php echo $uid;?>">
													<?php echo $masonary_text; ?>
												</div>
												<?php } ?>
											</div>
											<?php if( $grid_style !== 'features' ) { ?>
							...
							<?php if( $masonary_linktype ) { ?></a><?php } ?>
							...
							?>
							<div style="display: none;max-width: 700px;" id="popop-form-<?php echo $uid;?>-<?php echo $link_item;?>" class="button-popop-form">
					...
					?>
						<div class="grid-item">
							...
							<div class="grid-item-inner">
								...
								<div class="flex_item_title_over">
									<div class="flex_item_title_over_inner"><?php echo $masonary_title; ?></div>
								</div>
								<?php } ?>
								<?php if( $masonary_subtitle || $masonary_title ) { ?>
			...
			<?php } elseif( $grid_source == 'page' ) { 
				$page_grid_style = get_field( 'fg_pgs' );
				$page_grid_pages = get_field( 'fg_pgp' );
				$page_grid_image_size = get_field( 'fg_pis' );
			?>

			<div class="masonary_grid_link_wrap masonary_grid_<?php echo $grid_source;?> masonary_grid-<?php echo $uid;?> page_grid_style_<?php echo $page_grid_style;?>">
				<div class="masonary_grid<?php if( $grid_slider ) { ?> slider-container<?php } ?><?php if( $grid_slidermo ) { ?> slider-container-mo<?php } ?>">
					<div class="layout <?php if( $grid_slider ) { ?>slider-wrapper<?php } else { ?>row-flex center-xs<?php } ?>">
					<?php 
					if( $page_grid_pages ) {	
					foreach( $page_grid_pages as $grid_page): 
				        $permalink = get_permalink( $grid_page->ID );
				        $title = get_the_title( $grid_page->ID );
					
					?>
						<div class="grid-item <?php if( $grid_slider ) { ?>slider-slide<?php } else { ?>col-xs-<?php echo $m_xs_cols; ?> col-sm-<?php echo $m_sm_cols; ?> col-md-<?php echo $m_md_cols; ?> col-lg-<?php echo $m_lg_cols; ?><?php } ?>">
							<a href="<?php echo $permalink; ?>" class="img_info_link">
								<?php if( $page_grid_style == 'article-split' ) { ?>
								<div class="grid-item-inner row-flex">
									<div class="grid-item-inner-img col-xs-12 col-sm-6">
										<?php echo get_the_post_thumbnail($grid_page->ID, $page_grid_image_size); ?>
									</div>
									<div class="grid-item-inner-con col-xs-12 col-sm-6">
										<div class="grid-item-inner-con-inn">
											<div class="grid-item-title"><?php echo $title; ?></div>
											<?php 
											$excerpt = get_field('p_sum', $grid_page->ID);
											if( $excerpt ) { 
											...
											<button class="section_readmore_link watch_btn hoverLink"><?php _e('Read More', 'generatepress_tkm'); ?></button>
											</div>
										</div>
									</div>
								</div>
								<?php } elseif( $page_grid_style == 'article-mix' ) { 
									$hide_date = get_field( 'fg_hda' );
								?>
								<div class="grid-item-inner">
									<div class="grid-item-inner-img">
										...
										<div class="grid-item-inner-con">
											<div class="grid-item-inner-con-inn">
												<?php if (!$hide_date) { ?>
												<div class="grid-item-date"><?php echo get_the_date('d.m.Y', $grid_page->ID); ?></div>
												<?php } ?>
												<div class="grid-item-title"><?php echo $title; ?></div>
												<?php 
												$excerpt = get_field('p_sum', $grid_page->ID);
												...
												<?php } ?>
												<div class="grid-item-btn">
												<button class="section_readmore_link watch_btn hoverLink"><?php _e('More info', 'generatepress_tkm'); ?></button>
												</div>
											</div>
										</div>
									</div>
								...
								<div class="grid-item-inner">
									<div class="grid-item-inner-img">
										...
										<div class="article_hover_btn">
											<div class="article_hover_btn_inner"><?php _e('Read More', 'generatepress_tkm'); ?></div>
										</div>
										<?php } ?>
										<?php echo get_the_post_thumbnail($grid_page->ID, $page_grid_image_size); ?>
									</div>
									<div class="flex_masonary_title">
										<div class="flex_masonary_title_inner"><?php echo $title; ?></div>
										<?php 
			...
			$product_grid_style = get_field( 'fg_prgs' );
			$product_grid_pages = get_field( 'fg_prgp' );
			$product_grid_image_size = get_field( 'fg_pris' );
			?>
			<div class="masonary_grid_link_wrap masonary_grid_<?php echo $grid_source;?> masonary_grid-<?php echo $uid;?> page_grid_style_<?php echo $product_grid_style;?>">
								...
								<?php if( $product_grid_style == 'simple' ) { ?>
									<div class="product_grid_item_container">
										...
										<div class="product_grid_item_title_wrap row-flex">
											<div class="product_grid_item_title col-xs-12"><?php the_title(); ?></div>
			...
			$gallry_img_size = get_field('fg_igs');	
			$gallery_images = get_field('fg_ig');
			$grid_gallery_flayout = get_field('fg_igfl');
			?>

			<div class="masonary_grid_link_wrap masonary_grid-<?php echo $uid;?>">
				<div class="masonary_grid<?php if( $grid_slider ) { ?> slider-container<?php } ?><?php if( $grid_slidermo ) { ?> slider-container-mo<?php } ?>">
					<?php if( $grid_type == 'box-layout' ) { ?>
					<div class="layout grid_cols_<?php echo $grid_count; ?> <?php if( $grid_slider ) { ?>slider-wrapper<?php } else { ?>row-flex center-xs<?php } ?>">
			...
			<div class="section_btn grid_btn section_readmore_link_wrap">
				...
				?>					
				<a class="full_content_link" data-fancybox data-src="#popop-form-<?php echo $uid;?>" href="javascript:;">
				<?php endif; ?>
				...
				?>
				<div style="display: none;max-width: 500px;" id="popop-form-<?php echo $uid;?>" class="popop-feature button-popop-form">
					<div class="button-popop-form-wrap">
						<div class="button-popop-form-row">
							<div class="button-popop-form-col form-image col-xs-12">
		...
		jQuery(function($) {
			...
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(3)').addClass('span-3');
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(5)').addClass('span-2');
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(10)').addClass('span-2');
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(13)').addClass('span-2');
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(17)').addClass('span-3');
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(21)').addClass('span-2');
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(26)').addClass('span-3');
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(40)').addClass('span-3');
			$('#section-<?php echo $uid;?> .grid-layout .grid-item:nth-child(45)').addClass('span-3');
			<?php endif; ?>
			<?php if( $grid_type == 'flex-layout' && $grid_gallery_flayout == 'layout1' || $grid_source == 'manual' && $grid_type == 'flex-layout' ): ?>
				$('#section-<?php echo $uid;?> .flex-layout .layout').addClass('row-flex');
	
				var elems = $("#section-<?php echo $uid;?> .flex-layout .layout > .grid-item");
				var wrapper = $('<div class="col_layout col-xs-12 col-sm-6"><div class="row-flex">');
				var pArrLen = elems.length;
				for (var i = 0;i < pArrLen;i+=2){
				    elems.filter(':eq('+i+'),:eq('+(i+1)+')').wrapAll(wrapper);
				};
				$('#section-<?php echo $uid;?> .flex-layout .col_layout:nth-child(4n+1)').addClass('row-a');
				$('#section-<?php echo $uid;?> .flex-layout .col_layout:nth-child(4n+2)').addClass('row-a');
				$('#section-<?php echo $uid;?> .flex-layout .col_layout:nth-child(4n+3)').addClass('row-b');
				$('#section-<?php echo $uid;?> .flex-layout .col_layout:nth-child(4n+4)').addClass('row-b');
	
				$('#section-<?php echo $uid;?> .flex-layout .grid-item').addClass('col-xs-12');
				//$('.flex-layout .col_layout:nth-child(odd) .grid-item').addClass('col-sm-6');
			<?php endif; ?>

			<?php if( $grid_type == 'flex-layout' && $grid_gallery_flayout == 'layout2'): ?>
				$('#section-<?php echo $uid;?> .source_gallery.flex-layout .layout').addClass('row-flex');
	
				$('#section-<?php echo $uid;?> .source_gallery.flex-layout .grid-item:nth-child(5n+1)').addClass('col-xs-12 col-sm-6');
				$('#section-<?php echo $uid;?> .source_gallery.flex-layout .grid-item:nth-child(5n+2)').addClass('col-xs-12 col-sm-6');
				$('#section-<?php echo $uid;?> .source_gallery.flex-layout .grid-item:nth-child(5n+3)').addClass('col-xs-12 col-sm-2');
				$('#section-<?php echo $uid;?> .source_gallery.flex-layout .grid-item:nth-child(5n+4)').addClass('col-xs-12 col-sm-5');
				$('#section-<?php echo $uid;?> .source_gallery.flex-layout .grid-item:nth-child(5n+5)').addClass('col-xs-12 col-sm-5');
				$('#section-<?php echo $uid;?> .source_gallery.flex-layout .grid-item').addClass('col-xs-12');
